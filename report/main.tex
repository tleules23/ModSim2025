...
5. briefly describe that we always apply monte carlo, and what kind of statistics we are looking at

6. explain we are also using logging for deug and so on and just show a brief snippet

\section{Starvation Avoidance}

Deadlocks are not the only issues that can occur during the simulation runs. Because of the mostly stochastic nature of the dining philosopher problem, given by the unordered sequence of chopstick aquisitions by the philsophers, it is possible that each participant in the simulation run indefinitely waits for his turn to eat. A fitting example would be the case where both neighbours of the same philosopher always take the chipstocks first. Long enough waiting times can lead to starvation which for the dining philosophers problem is considered to have failed if even one philosopher starves.
This possibility of starvation requires the addition of an mechanism which ensures fairness between the members for aquiring the chopsticks.
